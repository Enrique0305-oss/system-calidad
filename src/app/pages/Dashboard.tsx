          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1">
            {Array.from({ length: 13 }, (_, i) => i + 1).map(n => (
              <div
                ...
                onClick={() => n === 1 ? navigate('/mp/ingreso') : n <= 11 ? navigate('/proceso') : n === 12 ? navigate('/pt/ingreso') : navigate('/pt/rastreo')}
                title={`Paso ${n}`}
              >
                {n}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => navigate('/proceso')} style={{ background: 'none', border: 'none', color: '#1E3A5F', fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 2, fontFamily: 'Inter,sans-serif', fontWeight: 600 }}>
