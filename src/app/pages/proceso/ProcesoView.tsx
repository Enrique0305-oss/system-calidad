      ...
      {view === 'diagram' && (
        <div style={{ background: '#0F2744', borderRadius: 16, overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,0,0,0.2)' }}>
          ...
          <div className="relative">
            <img src={processDiagram} alt="Diagrama de proceso de producción de yogurt" style={{ width: '100%', display: 'block', maxHeight: 520, objectFit: 'contain', background: '#fff' }} />

            {/* Overlay step buttons */}
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', pointerEvents: 'none' }}>
              {/* Row 1 steps: 1-6 */}
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '8px 24px', pointerEvents: 'auto' }}>
                {pasos.slice(0, 6).map(paso => {
                  const cfg = ESTADO_CFG[paso.estado];
                  return (
                    <button key={paso.id} onClick={() => setSelectedPaso(paso)}
                      style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '6px 8px', borderRadius: 8, cursor: 'pointer', border: `2px solid ${cfg.dot}`, background: `${cfg.dot}25`, backdropFilter: 'blur(4px)', transition: 'all 0.15s', minWidth: 60 }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'scale(1.1)'; (e.currentTarget as HTMLElement).style.boxShadow = `0 4px 12px ${cfg.dot}60`; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'scale(1)'; (e.currentTarget as HTMLElement).style.boxShadow = 'none'; }}
                    >
                      <div style={{ width: 20, height: 20, borderRadius: '50%', background: cfg.dot, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff' }}>{paso.id}</div>
                      <span style={{ fontSize: 9, fontWeight: 700, color: '#fff', textAlign: 'center', lineHeight: 1.2, maxWidth: 50 }}>{paso.nombre}</span>
                    </button>
                  );
                })}
              </div>
              ...
              {/* Step 13 row */}
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px 24px', pointerEvents: 'auto' }}>
                {pasos.slice(12).map(paso => {
                  const cfg = ESTADO_CFG[paso.estado];
                  return (
                    <button key={paso.id} onClick={() => setSelectedPaso(paso)}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, cursor: 'pointer', border: `2px solid ${cfg.dot}`, background: `${cfg.dot}25`, backdropFilter: 'blur(4px)', transition: 'all 0.15s' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = 'scale(1.05)'; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = 'scale(1)'; }}
                    >
                      <div style={{ width: 20, height: 20, borderRadius: '50%', background: cfg.dot, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff' }}>{paso.id}</div>
                      <span style={{ fontSize: 10, fontWeight: 700, color: '#fff' }}>{paso.nombre}</span>
                    </button>
                  );
                })}
              </div>
